...
if (!isDedicated) then {
	...
	//fn_blockDirect =            compile preprocessFileLineNumbers "\z\addons\br_assets\scripts\fn_blockdirect.sqf";
  ...
  if (!isDedicated) then
	...
	dayz_spaceInterrupt = {
		private ["_dikCode", "_handled","_displayg","_startpos"];
		...
		*/
		...
		if (_dikCode in actionKeys "Turbo") then {_handled = true};

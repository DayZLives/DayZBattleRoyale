...
if (!isDedicated) then {	
	...
	BR_PARA = {
		...
		_smokeRed attachto [player, [0,0,0]];
		while {_doSmoke} do {
	...
	onPreloadFinished {
		...
		_firstlocation = [getMarkerPos "center",random 2500,random 360,false] call SHK_pos;
		_finallocation = [_firstlocation,random 700,random 360,false] call SHK_pos;
	
		player setPos _finallocation;

		//br_start_lock = [] execVM "br_code\scripts\fn_startlock.sqf";
		0 = [25,300,200,100,40] execvm "br_code\scripts\tpw_park.sqf";
		0 = [100,500,10,1,5] execvm "br_code\scripts\tpw_fall.sqf";
		0 = [4,0.8,0.85,0.95] execvm "tpw_bleedout.sqf";
